...
<main class="container">
    ...
    if (isset($_POST['update']) || isset($_POST['update_redirect'])) {
        ...
        if (!empty($_FILES['icon']['name'])) {
            ...
            // If no image error, handle image
            if (empty($errors)) {
        ...
        if (empty($errors)) {
            ...
            $query = "UPDATE game_table SET 
                        ...
                        dupes_name='$dupes', 
                        skill_name='$skill', 
                        stat_amplifier='$amplifier',
                        game_icon='$newfilename'
                    WHERE id=$id";
            ...
            if ($result) {
                if (isset($_POST['update_redirect'])) {
                    echo "<script>alert('Game Added!'); window.location.href = 'index.php?page=categories/edit&id=".$id."';</script>";
                } else {
                    echo "<script>alert('Game Updated!'); document.location.href = 'index.php?page=game/read&id=".$id."';</script>";
                }
            } else {
    ...
    <form action="" method="post" enctype="multipart/form-data" class="text-white font1 mt-4">
        ...
        <?php if (!empty($current_img)) : ?>
            <center>
                <img src="<?= BASE_URL ?>/uploads/<?= $current_img ?>" alt="Current Icon" style="max-width: 130px;" class="h-auto my-2">
            </center>
        <?php endif; ?>
        <div class="row mb-3">
            ...
            <div class="col-sm-10">
                ...
                <small>Leave if didn't change the image</small>
            </div>
        </div>
